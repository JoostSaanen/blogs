...
There is some real excitement going on in the elixir community these last couple of weeks due to the fact that Chris McChord made his PhoenixLiveView repo available to the public. People have been experimenting wildly with the imense power that comes with Phoenix Live View. If you are totally new to LiveView and haven't heard about it, be sure to check out the initial blog post on the Dockyard website and also this talk by Chris.

Now that we have a common understanding of what LiveView is, let's see what we can explore today using LiveView. Most things I have seen thusfar have been standalone interactive little projects that contain either some nifty animations and/or some interactivity using forms. Very awesome stuff! I'd like to take the concept of LiveView to a slightly other direction and use it for something that is more real world like... (change this!). One of the use cases Chris has mentioned is dashboards and that is exactly what I would like to explore. And with the upcoming ElixirConfEU in Prague, let's see if we can actually get something useful of the ground.

### A LiveView Template

Setting uop a project for LiveView is explained step-by-step in the LiveView Repository, so we could of course just create a Phoenix Project and follow those steps and set everything up like that. But, 
I am always looking for ways to make sure we as a community don't have to do all kinds of manual tasks over and over again and I recently came accross a nice little library created by Dave Thmomas that provides a really nice and clean way to create template projects. You can read more about that `here` and be sure to watch the video made by Dave about this.

So, as a convenience I have created a phx_live_view template so that everyone can have a fresh Phoenix Live View project wuick and easy. In just 4 easy commands you'll have your own Phoenix Live View project up and running with an example clock on the homepage.
...
For my dashboard I wanted to have some boilerplate styling and libraries to make my design work a little easier (because that is just not my cup-of-thea). So I created an additional template that sets up a few other things. Namely bootstrap and sass and some initial dashboard layout.

You can ofcourse use this template as a starting just as easily as the basic one:
...
If you start up this either of these templates you'll see that it already has a LiveView clock running. 

### Countdown To ElixirConf

With the upcoming ElixirConfEU in Prague, let's see if we can get some more realtime data on the screen that relates to this upcoming event. Let's start by creating a countdown timer to the start of the conference!

Adding a new LiveView component can be done in two simple steps.

1. create a LiveView in `lib/yourproject_web/live/LiveViewModule.ex`
...
Here is the exmple from the clock that is in the template.
```elixir
  <%= live_render(@conn, ElixirConfLiveViewWeb.Clock) %>
```

Let's create the countdown timer. I'll use the awesome Timex library to make my life a little easier. :) 
...
And here is the ViewModule implementation of the countdown timer. Once the livesocket is connected it fires off a timer that send a message to the process every second to get the current duration to the start of ElixirConf. That triggers and update to the socket.assigns, which in it's turn triggers the render function to rerender the component on the page. Sounds too easy? Well it is too easy. Great stuff!.
...
Cool now we at least know how long we have to wait... :) Now let's see if we can integrate some third-party data into our dashboard. As this post is about PhoenixLiveView which is officially not released yet, wouldn't it be nice to get some stats from github to see the issue count, stars, forks and the current open PRs to see how close we are to an actual release, right? 
Getting data from github is a breeze if we use the Tentacat library, so let's add that to the mix!
...
You'll notice that I am actually authenticating to fetch the data. You can actually get this data without authenticating, but you'll hit the rate limit very quickly, so that is why I chose to authenticate here. Using Tentacat, this is very easy, so that doesn't really make things more complicated at all.
What I also wanted to do here is to add a countdown timer to the talk by Chris @ ElixirConf as I'm hoping that he might anounce an official release there... :) 
...
Ok, so far so good. Currently, aside from the clock, there is not so much real-time data going on. r at least you'd have to wait some time for the stars and issues to change, but it's a start. You could of course star/unstar the repo yourself(and wait 30 seconds) to see it change. 
...
It should be pretty obvious that we shouldn't be fetching the data from the API in the ViewModule itself. In the unstructions Chris mentions that we shouldn't load data in the template, but I think the same statement holds for the whole ViewModule, maybe not all the time, but especially when connecting to 3rd party services. So let's solve this problem by creating a service that polls the 3rd party for updates and pushes those updates down to the LiveView modules. This is exactly the kind of thing we could us the `Phoenix.PubSub` module for! This is used internally in Phoenix to handle the implementation of `Phoenix.Channels`, but also has an API to use it for anyting else that needs a publish-subscribe pattern.
...
Often I see people using `Process.send_after` to configure a timer-like scheduled function call, but that functionality is actually built into GenServer as a :timeout callback. So you just have to pass the timeout value in the init function and configure a `handle_info/2` function to catch that and do whatever you please. We could now create a separate module to implement our PubSub functionallity, but we could also use the one that is already running in the Endpoint. So all we have to do to publish our new state is broadcast a message in a topic using `Endpoint.broadcast/3`. In the case above I'm broadcasting the message `new_stats` to the `github` topic.
